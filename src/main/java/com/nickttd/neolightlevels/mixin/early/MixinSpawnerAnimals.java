...
public abstract class MixinSpawnerAnimals {
     ...
     */
    @Overwrite
    public int findChunksForSpawning(WorldServer world, boolean spawnHostile, boolean spawnPeaceful, boolean spawnAnimals) {
        if (!spawnHostile && !spawnPeaceful) {
            ...
            for (Object entityObj : entityList) {
                if (entityObj instanceof EntityLiving) {
                    ...
                    if (localMobCounts.containsKey(chunk)) {
                        ...
                        for (EnumCreatureType type : EnumCreatureType.values()) {
                            if (type.getCreatureClass().isInstance(entity)) {
            ...
            for (int k3 = 0; k3 < k; ++k3) {
                ...
                if ((!enumcreaturetype.getPeacefulCreature() || spawnPeaceful) && (enumcreaturetype.getPeacefulCreature() || spawnHostile) && (!enumcreaturetype.getAnimal() || spawnAnimals) && world.countEntities(enumcreaturetype, true) <= enumcreaturetype.getMaxNumberOfCreature() * eligibleChunksForSpawning.size() / 256) {
                    ...
                    while (iterator.hasNext()) {
                        ...
                        if (!((Boolean)eligibleChunksForSpawning.get(chunkcoordintpair1)).booleanValue()) {
                            ...
                            for (int centerAttempt = 0; centerAttempt < 3; ++centerAttempt) {
                                ChunkPosition chunkposition = pickRandomChunkPosition(world, chunkcoordintpair1.chunkXPos, chunkcoordintpair1.chunkZPos);
                                int centerX = chunkposition.chunkPosX;
                                int centerY = chunkposition.chunkPosY;
                                int centerZ = chunkposition.chunkPosZ;

                                if (!world.getBlock(centerX, centerY, centerZ).isNormalCube() && world.getBlock(centerX, centerY, centerZ).getMaterial() == enumcreaturetype.getCreatureMaterial()) {
                                    ...
                                    for (int packAttempt = 0; packAttempt < packSize; ++packAttempt) {
                                        ...
                                        for (int posAttempt = 0; posAttempt < 4 && !spawned; ++posAttempt) {
                                            ...
                                            if (canCreatureTypeSpawnAtLocationHelper(enumcreaturetype, world, tryX, tryY, tryZ)) {
                                                float f = (float)tryX + 0.5F;
                                                float f1 = (float)tryY;
                                                float f2 = (float)tryZ + 0.5F;
                                                if (world.getClosestPlayer((double)f, (double)f1, (double)f2, 24.0D) == null) {
                                                    ...
                                                    if (f6 >= 576.0F) {
                                                        if (spawnlistentry == null) {
                                                            spawnlistentry = world.spawnRandomCreature(enumcreaturetype, tryX, tryY, tryZ);
                                                            if (spawnlistentry == null) {
                                                                break;
                                                            }
                                                        }
                                                        ...
                                                        try {
                                                            entityliving = (EntityLiving)spawnlistentry.entityClass.getConstructor(new Class[] {net.minecraft.world.World.class}).newInstance(new Object[] {world});
                                                        ...
                                                        if (canSpawn == Result.ALLOW || (canSpawn == Result.DEFAULT && entityliving.getCanSpawnHere())) {
                                                            ...
                                                            if (mobCounts != null) {
                                                                mobCounts.put(enumcreaturetype, mobCounts.get(enumcreaturetype) + 1);
                                                            }
                                                            ++mobsSpawnedThisPack;
                                                            ++i;
                                                            spawned = true;
                                                            if (mobsSpawnedThisPack >= ForgeEventFactory.getMaxSpawnPackSize(entityliving)) {
                                                                break;
                                                            }
                                                        }
                                                    }
    ...
    private static boolean canCreatureTypeSpawnAtLocationHelper(EnumCreatureType type, World world, int x, int y, int z) {
